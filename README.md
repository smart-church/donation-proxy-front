...
Общая загрузка файлов для этапов B–D подготовлена в `frontend/src/components/FileUploader.jsx`: допустимые расширения (включая `.doc`/`.docx`) и лимиты загружаются из API. В редакторы компонент подключается на соответствующем этапе.
