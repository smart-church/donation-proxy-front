...
## Выпуск 14 сентября 2026: участники и файлы

На `dev.evman.ru` опубликованы backend `2efff09` и UI `f776cb7`, сборка
`main.a8e260a5.js`. Применены миграции 0012–0014, обновлены статика и worker.
Резервная копия сайта и БД: `~/deploy-backups/donation-release-20260914-230331/`.

В серверный `src/.env` добавлены только следующие настройки; остальные значения
и hosting-файлы сверены с резервной копией и сохранены:

```dotenv
FILE_ANTIVIRUS_EXECUTABLE=/usr/bin/clamscan
FILE_ANTIVIRUS_REQUIRED=True
FILE_SCAN_TIMEOUT=30
PUBLIC_FILE_UPLOAD_ENABLED=False
```

ClamAV пока не работает: `clamscan` завершился с кодом 2 из-за нехватки памяти
при загрузке устаревшей базы, `clamdscan` также вернул 2. Публичные загрузки
выключены; загрузки организатора требуют успешной проверки и тоже будут
отклоняться до исправления сканера. Для включения нужна помощь хостинга либо
перенос проверки в подходящее окружение. Не включать загрузки простым отключением
обязательной проверки.

Настроена почасовая очистка через `~/run-dev-evman-cleanup.sh` (17-я минута часа).
Существующие cron-задания сохранены. Приватное хранилище:
`~/.local/share/donation-proxy/files`, права 700; `.env` — 600.

При публикации архивов явно проверяйте права: каталог сайта и каталоги `public/`
должны быть доступны веб-серверу (755), публичные файлы — 644. Не переносите
приватные права staging-каталога на DocumentRoot. Например, для статики
используйте `rsync -a --chmod=D755,F644`, сохраняя файлы запуска и старые хешированные
assets. Эти права не применять к `.env`, резервным копиям и приватному хранилищу.

После перезапуска проверены HTTPS `/`, `/events`, JS-сборка и Django-скрипт — 200;
API без авторизации — 401 JSON. Ручное создание/чтение участника и удаление
тестового мероприятия проверены через Django APIClient в транзакции с откатом,
без сохранения тестовых данных и отправки писем.
